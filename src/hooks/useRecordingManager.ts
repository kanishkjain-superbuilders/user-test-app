...
export interface RecordingManager {
  ...
  startRecording: (recordingId: string, options: RecordingOptions) => Promise<void>;
...
export function useRecordingManager(): RecordingManager {
  ...
  const toggleMute = useCallback((type: 'mic' | 'cam') => {
    const stream = type === 'mic' ? streamsRef.current.audio : streamsRef.current.video;
    if (!stream) return;

    const tracks = type === 'mic' ? stream.getAudioTracks() : stream.getVideoTracks();
   ...
   * Get mute state
   */
  const getMuteState = useCallback((): { micMuted: boolean; camMuted: boolean } => {
