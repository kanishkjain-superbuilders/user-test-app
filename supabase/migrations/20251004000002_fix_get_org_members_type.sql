-- Fix the type mismatch in get_org_members function
...
RETURNS TABLE (
    ...
    user_email VARCHAR(255)  -- Changed from TEXT to match auth.users.email type
)
SECURITY DEFINER
AS $$
BEGIN
...
-- Grant execute permission to authenticated users (re-grant after recreating)
